"""Panda Lissajous force-control environment for HITL training.

This is the EXACT same logic as panda_env_lissajous_delta.py but adapted for HITL:
- X-Y: Perfect Lissajous trajectory (math-controlled)
- Z-force: AI learns via HITL corrections
"""

import os
import numpy as np
import gymnasium as gym
from gymnasium import spaces
import raisimpy as raisim

from config import DEFAULT_CONFIG


class PandaLissajousHitlEnv(gym.Env):
    """Panda environment combining perfect Lissajous trajectory + learned Z-force control."""
    
    def __init__(self, config=None):
        self.config = config or DEFAULT_CONFIG
        self.physics_cfg = self.config.physics
        self.robot_cfg = self.config.robot
        self.timestep = self.physics_cfg.control_dt
        
        # 1D ACTION SPACE: AI outputs [-1, 1] which we dynamically scale into a delta force
        self.action_space = spaces.Box(-1.0, 1.0, shape=(1,), dtype=np.float32)
        
        # 20D OBSERVATION SPACE: q(7) + v(7) + force(3) + dist(1) + current_z_force(1) + error_sum(1) = 20
        self.observation_space = spaces.Box(-np.inf, np.inf, shape=(20,), dtype=np.float32)
        
        # --- RAISIM SETUP ---
        raisim.World.setLicenseFile(os.path.dirname(os.path.abspath(__file__)) + "/../../rsc/activation.raisim")
        self.world = raisim.World()
        self.world.addGround()
        self.world.setTimeStep(self.physics_cfg.physics_dt)
        
        self.panda = self.world.addArticulatedSystem(os.path.dirname(os.path.abspath(__file__)) + "/../../rsc/panda/urdf/panda.urdf")
        self.panda.setName("panda")

        # --- BOX SETUP ---
        self.box_center_x = self.robot_cfg.box_center_x
        self.box_center_y = self.robot_cfg.box_center_y
        self.box_pos = [self.box_center_x, self.box_center_y, 0.1]
        self.box = self.world.addBox(0.4, 0.4, 0.5, 100.0)
        self.box.setPosition(*self.box_pos)
        self.box.setBodyType(raisim.BodyType.STATIC)

        # --- END EFFECTOR & TARGETS ---
        self.target_force_mag = self.robot_cfg.target_force_mag
        self.ee_frame_idx = self.panda.getFrameIdxByName(self.robot_cfg.ee_frame_name)
        self.ee_frame_name = self.robot_cfg.ee_frame_name
        self.ee_offset = self.robot_cfg.ee_offset
        
        self.t = 0.0

        # Safe starting joint configuration
        self.contact_q = np.array(([-1.03996814, -0.24488271, -0.13182964, -2.39703664,  0.13306204,
        2.33681407,  0.69235052]))
        
        # HITL-specific state
        self.human_accumulated_force = 0.0
        self.commanded_force_z = 0.0
    def _get_lissajous_target(self):
        """Generates a gentle Figure-8 that starts EXACTLY at the center."""
        f_x = self.robot_cfg.lissajous_freq_x
        f_y = self.robot_cfg.lissajous_freq_y
        A = self.robot_cfg.lissajous_amplitude_x
        B = self.robot_cfg.lissajous_amplitude_y
        
        # REMOVED delta so sin(0) = 0 at the start
        target_x = self.box_center_x + A * np.sin(2.0 * np.pi * f_x * self.t)
        target_y = self.box_center_y + B * np.sin(2.0 * np.pi * f_y * self.t)
        
        return np.array([target_x, target_y])

    def reset(self, seed=None, options=None):
        super().reset(seed=seed)
        self.t = 0.0 
        
        self.current_force_z = self.robot_cfg.max_commanded_force
        self.error_sum = 0.0          # Integral error accumulator
        
        # HITL reset
        self.human_accumulated_force = 0.0
        self.commanded_force_z = self.current_force_z
        
        self.panda.setGeneralizedCoordinate(self.contact_q)
        self.panda.setGeneralizedVelocity(np.zeros(7))
        self.panda.setGeneralizedForce(np.zeros(7))
        return self._get_obs(), {}
    def step(self, action):
        self.t += self.timestep

        # ==========================================
        # 2. THE AI (Consistent Delta + Z-Damping)
        # ==========================================
        raw_action = float(action[0])

        # Positive action means "push harder", matching the HITL UI guidance.
        delta_force = -raw_action * abs(self.robot_cfg.min_commanded_force) * self.timestep

        self.current_force_z = np.clip(
            self.current_force_z + delta_force,
            self.robot_cfg.min_commanded_force,
            self.robot_cfg.max_commanded_force,
        )
        
        # Update commanded force for HITL bookkeeping
        self.commanded_force_z = self.current_force_z
        
        for _ in range(self.physics_cfg.substeps):
            ee_pos = self.panda.getFramePosition(self.ee_frame_idx)
            ee_vel = self.panda.getFrameVelocity(self.ee_frame_idx)

            # ==========================================
            # 1. THE VIRTUAL SPRING (Math controls X/Y)
            # ==========================================
            target_xy = self._get_lissajous_target()
            Kp = self.robot_cfg.kp_trajectory_xy
            Kd = self.robot_cfg.kd_trajectory_xy

            force_x = (Kp * (target_xy[0] - ee_pos[0])) - (Kd * ee_vel[0])
            force_y = (Kp * (target_xy[1] - ee_pos[1])) - (Kd * ee_vel[1])

            # Physical Z-damping acts as a shock absorber against the box.
            Kd_z = 25.0
            damping_z = -Kd_z * ee_vel[2]

            final_force_z = self.current_force_z + damping_z
            desired_force_at_ee = np.array([force_x, force_y, final_force_z])

            # ==========================================
            # 3. PHYSICS EXECUTION
            # ==========================================
            J = self.panda.getDenseFrameJacobian(self.ee_frame_name)
            tau_action = np.matmul(J[:3, :].T, desired_force_at_ee)
            tau_grav = self.panda.getNonlinearities(self.world.getGravity())
            tau_damping = -1.0 * self.panda.getGeneralizedVelocity()

            self.panda.setGeneralizedForce(tau_action + tau_grav + tau_damping)
            self.world.integrate()
        
        # ==========================================
        # 4. REWARD CALCULATION
        # ==========================================
        new_ee_pos = self.panda.getFramePosition(self.ee_frame_idx)
        dist_to_surface = max(0, new_ee_pos[2] - self.robot_cfg.surface_height - self.ee_offset)
        
        new_force = self._get_contact_force()
        new_force_mag = np.linalg.norm(new_force)
        force_error = abs(new_force_mag - self.target_force_mag)

        # Keep integral error for observation, but clip it tightly
        signed_error = new_force_mag - self.target_force_mag
        self.error_sum += signed_error * self.timestep
        self.error_sum = np.clip(self.error_sum, -2.0, 2.0)

        normalized_error = force_error / max(1e-6, self.target_force_mag)
        action_penalty = 0.02 * abs(raw_action)
        if dist_to_surface > 0.002:
            reward = -100.0 * dist_to_surface - normalized_error - action_penalty
        else:
            # Dense signal across the whole force range: 0N is bad, 10N is best.
            reward = 1.0 - normalized_error - action_penalty
        
        # HITL info dict
        info = {
            'force_mag': float(new_force_mag),
            'force_error': float(force_error),
            'commanded_force_z': float(self.commanded_force_z),
            'human_accumulated_force': float(self.human_accumulated_force),
            'on_surface': bool(dist_to_surface <= 0.002),
            'reward': float(reward),
        }
            
        return self._get_obs(), reward, False, False, info
    def _get_obs(self):
        q = self.panda.getGeneralizedCoordinate()
        v = self.panda.getGeneralizedVelocity()
        f = self._get_contact_force()
        ee_pos = self.panda.getFramePosition(self.ee_frame_idx)
        dist = max(0, ee_pos[2] - self.robot_cfg.surface_height - self.ee_offset)
        
        # Add the agent's current Z-force and the integral error to the observation
        return np.concatenate([q, v, f, [dist], [self.current_force_z], [self.error_sum]]).astype(np.float32)

    def _get_contact_force(self):
        contacts = self.panda.getContacts()
        total_force = np.zeros(3)
        for c in contacts:
            if not c.skip():
                total_force += np.array(c.getImpulse())
        return total_force / self.physics_cfg.physics_dt
    
    def update_human_force_delta(self, assist_action: float):
        """HITL interface: update human accumulated force from assistance."""
        # assist_action is in [-1, 1], map to force delta
        force_delta = -assist_action * abs(self.robot_cfg.min_commanded_force) * self.timestep
        self.human_accumulated_force += force_delta
        self.human_accumulated_force = np.clip(
            self.human_accumulated_force,
            self.robot_cfg.min_commanded_force,
            self.robot_cfg.max_commanded_force,
        )
    
    def close(self):
        """Release the Raisim world."""
        if hasattr(self, 'world'):
            del self.world


if __name__ == "__main__":
    # Simple test
    env = PandaLissajousHitlEnv()
    obs, _ = env.reset()
    print(f"Initial obs shape: {obs.shape}")
    
    for step in range(100):
        action = np.array([np.sin(step * 0.05) * 0.3])
        obs, reward, terminated, truncated, info = env.step(action)
        if step % 20 == 0:
            print(f"Step {step}: Reward={reward:.4f}, Force={info['force_mag']:.2f}N, "
                  f"CmdForce={env.commanded_force_z:.2f}N")
    
    env.close()
