...
from online_hitl_wrapper import OnlineHitlWrapper
...
class OnlineHitlTrainer:
    ...
    def initialize(self) -> None:
        ...
        try:
            ...
            if self.config.robot.enable_lissajous:
                logger.info("  → Using Lissajous trajectory + force control mode")
                self.env = PandaLissajousHitlEnv(config=self.config)
            else:
                logger.info("  → Using force control only mode")
...
def main():
    ...
    # Enable Lissajous trajectory tracking:
    config.robot.enable_lissajous = True
    #
    # Adjust Lissajous parameters:
    # config.robot.lissajous_freq_x = 0.2   # Hz
    # config.robot.lissajous_freq_y = 0.4   # Hz
    # config.robot.lissajous_amplitude_x = 0.03  # meters
    # config.robot.lissajous_amplitude_y = 0.03  # meters
    #
