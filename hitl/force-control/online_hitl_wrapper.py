...
from working.config import DEFAULT_CONFIG, HitlConfig
...
class OnlineHitlWrapper:
    ...
    def _get_executed_action(self, ai_action: float, human_action: float) -> float:
        ...
        assist_gain = float(getattr(self.config.human_input, "assist_gain", 1.0))
    ...
    def step_hitl(self, obs: np.ndarray) -> Tuple[np.ndarray, float, bool, bool, Dict, Dict]:
        ...
        # The observation stores the measured contact force in obs[-5:-2].
        force_mag = float(np.linalg.norm(obs[-5:-2]))
        ...
        is_intervening = abs(human_action) > self.config.human_input.deadzone
        if is_intervening:
        ...
        assist_force_delta = assist_action * 20.0 * 0.01
